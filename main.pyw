...
class Main_App(CTk.CTk):
    ...
    def __init__(self):
        ...
        mouse_listener = mouse.Listener(on_click=lambda x,y,button,pressed: self.display(x,y,button,pressed))
    ...
    def enter_settings(self):
        hwnd =  win32gui.FindWindow(None,'Cooldown Overlay: Settings') # Try to find Settings Window
    ...
    def change_key(self,key):
        if not key in self.hotkeys: return
        codecc=keyboard.KeyCode.from_char(self.current_key) # Takes Key and Cast as keyboard.KeyCode
        if key != codecc: # If the Key Pressed doesn't equal Current KeyCode, then set Key as Current Key
    ...
    def timed(self,key,parent):
        ...
        if parent == 'left': # If the Display was on the left side, Decrease [left_counter] by 1
            self.left_counter.set(self.left_counter.get()-1)

    # Displays the Cooldown on Click
    def display(self,x,y,button,pressed):
        if not pressed: return # If Click is Let Go, then return
        ...
        parent_name='left'
        if self.left_counter.get() > 5: # If Left has more than 5 displays, Set Parent to Right
            parent=self.right_frame
            parent_name='right'
        else:
            self.left_counter.set(self.left_counter.get()+1) # Else Increment Left Counter by 1

        keybind_name=self.saves['keybinds'][self.current_key] # Gets the Name of Keybind
        keybind_time=self.saves['cooldowns'][keybind_name] # Gets Cooldown  of Keybind
        Hotkey_Display(parent,keybind_name,keybind_time) # Displays Name and Cooldown Time
